...
- **接口自学习** — X 会不定期更换内部接口版本。插件会记录网页端自己使用的接口，因此接口更新后只需在 X 网页端手动加一次用户，插件即可自动恢复。
...
- 通过 X 网页端自身的 GraphQL 接口完成加入操作，复用浏览器登录态
- 页面主世界的脚本会记录 X 使用的接口版本与参数，实现上面提到的自学习

## 注意事项

- 使用的是 X 网页端的内部接口，**不是官方公开 API**。X 前端改版可能导致功能暂时失效。
- 请合理使用，避免高频批量操作。
- 仅供个人学习与自用。
